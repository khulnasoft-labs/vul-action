...
while getopts "a:b:c:d:e:f:g:h:i:j:k:l:m:n:o:p:q:r:s:t:u:v:z:" o; do
   case "${o}" in
       ...
       ;;
       t)
         export vulIgnores=${OPTARG}
       ;;
       u)
         export githubPAT=${OPTARG}
       ;;
       v)
         export vulConfig=${OPTARG}
       ;;
       z)
...
if [ $vulIgnores ];then
  ...
  ARGS="$ARGS --ignorefile ./vulignores"
fi
if [ $timeout ];then
...
vulConfig=$(echo $vulConfig | tr -d '\r')
...
if [ "${format}" == "sarif" ] && [ "${limitSeveritiesForSARIF}" != "true" ]; then
  ...
  vul --quiet ${scanType} --format sarif --output ${output} $SARIF_ARGS ${artifactRef}
elif [ $vulConfig ]; then
   echo "Running Vul with vul.yaml config from: " $vulConfig
   vul --config $vulConfig ${scanType} ${artifactRef}
   returnCode=$?
else
   echo "Running vul with options: vul ${scanType} ${ARGS}" "${artifactRef}"
   echo "Global options: " "${GLOBAL_ARGS}"
   vul $GLOBAL_ARGS ${scanType} ${ARGS} ${artifactRef}
   returnCode=$?
fi
